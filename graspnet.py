...
from modules import ApproachNet, GraspableNet, CloudCrop, SWADNet,fuseFeature_rgbd_pointCloud,ScaleAttention_Scale
...
class GraspNet(nn.Module):
    def __init__(self, cylinder_radius=0.05, seed_feat_dim=512, is_training=True):
        ...
        self.rgbd_feature_dim = 128
        self.num_depth = NUM_DEPTH
        self.num_angle = NUM_ANGLE
        self.M_points = M_POINT
        self.num_view = NUM_VIEW
        self.rgbd_backbone = SeResUNet(1, out_channels=self.rgbd_feature_dim)
        self.backbone = MinkUNet14D(in_channels=3, out_channels=self.seed_feature_dim, D=3)
        self.graspable = GraspableNet(seed_feature_dim=self.seed_feature_dim+self.rgbd_feature_dim)
        self.rotation = ApproachNet(self.num_view, seed_feature_dim=self.seed_feature_dim+self.rgbd_feature_dim, is_training=self.is_training)

        self.crop = CloudCrop(nsample=16,hmax=0.04 ,cylinder_radius=cylinder_radius, seed_feature_dim=self.seed_feature_dim+self.rgbd_feature_dim)
    ...
    def forward(self, end_points):
        ...
        # 在此加入分割网络，并进行特征匹配
        rgbd_features = self.rgbd_backbone(end_points['d'])
        seed_features = fuseFeature_rgbd_pointCloud(rgbd_features, seed_features,end_points['point_clouds_idx_in_rgbd'])#融合后的特征
        #特征融合之后 
        ...
        for i in range(B):
            ...
            fps_idxs = furthest_point_sample(cur_seed_xyz, self.M_points)###经过筛选后，再进行FPS选取点
        ...
        group_feature = self.crop(seed_xyz_graspable, seed_features_graspable, grasp_top_views_rot)#B,C,N

        end_points = self.swad(group_feature, end_points)
...
def pred_decode(end_points):
    ...
    for i in range(batch_size):
        grasp_center = end_points['xyz_graspable'][i].float()###这里还有多少个
