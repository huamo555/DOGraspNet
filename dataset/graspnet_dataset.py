...
class GraspNetDataset(Dataset):
    def __init__(self, root, grasp_labels=None, camera='kinect', split='train', num_points=20000,
        ...
        if split == 'train':
            ...
            self.sceneIds = list(range(100, 130))#原来起始位置是101，应该是错了，改成100
        ...
        for x in tqdm(self.sceneIds, desc='Loading data path and collision labels...'):
            for img_num in range(256):
                self.colorpath.append(os.path.join(root, 'scenes', x, camera, 'rgb', str(img_num).zfill(4) + '.png'))#加载RGB
    ...
    def get_data(self, index, return_raw_cloud=False):
        color = Image.open(self.colorpath[index])  # 读取图像
        depth_for_resize = Image.open(self.depthpath[index])  # 用于图像缩放
        transform_resize = transform.Resize([360, 640])
        color = transform_resize(color)
        depth_for_resize = transform_resize(depth_for_resize)
        color = np.array(color, dtype=np.float32) / 255.0  # 归一化
        depth_for_resize = np.array(depth_for_resize, dtype=np.float32)
        color = color.transpose(2, 0, 1)
        depth_for_resize = np.expand_dims(depth_for_resize, axis=0)
        rgbd = np.concatenate((color, depth_for_resize), axis=0)  # 4*360*640
        #创建和图像等大的数组用来前景标签对齐
        ...
        point_clouds_idx_in_rgbd__sampled = point_clouds_idx_in_rgbd_masked[idxs]
        ...
        ret_dict = {'point_clouds': cloud_sampled.astype(np.float32),
                    ...
                    'rgbd' : rgbd.astype(np.float32),  # (4,360,640) 
                    'd' : depth_for_resize.astype(np.float32)
                    }
        return ret_dict
...
def minkowski_collate_fn(list_data):
    ...
    #在下个函数中附加一行代码解决floor输入为int的问题，日后如果出现问题，关注此处
