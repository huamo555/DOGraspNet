...
parser.add_argument('--dataset_root', default='./dataset-data', required=False)#更改
parser.add_argument('--camera_type', default='realsense', help='Camera split [realsense/kinect]')




def run(start,end,gpu_num):
    os.environ['CUDA_VISIBLE_DEVICES'] = gpu_num # 指定卡，可以写在运行参数中。写成sh文件直接执行
    ...
    for scene_id in range(start,end) :
        ...
        labels = np.load(
                os.path.join(dataset_root, 'collision_label', 'scene_' + str(scene_id).zfill(4),
...
if __name__ == '__main__':
    ...
    save_path_root = os.path.join(dataset_root, 'graspness_label')#graspness
    ...
    for i in range(0,100,5):
      ...
      threading.Thread(target=run,args = (i,i+5,('1'),)).start()
